...
func (d *dbProxy) connect() {
	dsn := fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=disable",
		os.Getenv("GATEWAY"), //docker gateway
...
func GetEmploye(employe string, role string) bool {
	var count int64
	if role == "" {
		err := instance.db.Model(&models.Employe{}).Where("nom = ?", employe).Count(&count).Error
		if err != nil {
			logger.Error("Erreur lors de la vérification de l'employé: " + err.Error())
			return false
		}
		return count > 0
	}
	err := instance.db.Model(&models.Employe{}).Where("nom = ? AND role = ?", employe, role).Count(&count).Error
	if err != nil {
		logger.Error("Erreur lors de la vérification de l'employé: " + err.Error())
		return false
	}
